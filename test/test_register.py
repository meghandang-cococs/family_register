...
def test_read_current_LC_classes(test_family, test_student, test_student_class, test_current_classes_1):
    response = client.get("/student/1/read_current_LC_classes")
    assert response.status_code == status.HTTP_200_OK 

def test_read_current_LC_classes_fail(test_family, test_student, test_student_class, test_current_classes_1):
    response = client.get("/student/99/read_current_LC_classes")
    assert response.status_code == 404

def test_read_current_EP_classes(test_family, test_student, test_student_class, test_current_classes_2):
